...
class DatabaseConnection:
    ...
    def __init__(self, connection_type: str):
        ...
        # Fallback CSV directory (optional). Set env var FALLBACK_CSV_DIR to enable local CSV fallback
        self.fallback_dir = os.environ.get(
            'FALLBACK_CSV_DIR',
    ...
    def _create_native_connection(self) -> pyodbc.Connection:
        ...
        try:
            ...
            # Provide clearer, non-sensitive feedback to the UI and logs
            err_msg = str(e)
            try:
                ...
                _st.error(
                    ...
                    "Check network, port, driver, and credentials.\n"
                )
            except Exception:
                pass
            # Re-raise so callers can decide to fallback
            raise
    ...
    def _load_local_fallback(self) -> 'pd.DataFrame':
        """Attempt to load a local CSV from fallback_dir. Returns empty DataFrame if not found.

        Behavior: if FALLBACK_CSV_DIR contains multiple CSVs, returns the first one.
        This is a safety net so the dashboard can render with sample data when DB is down.
        """
        try:
            import pandas as pd
        except Exception:
        ...
        fallback = self.fallback_dir
        try:
            if not fallback or not os.path.isdir(fallback):
                return pd.DataFrame()

            csv_files = glob.glob(os.path.join(fallback, "*.csv"))
            if not csv_files:
                return pd.DataFrame()
    ...
    def execute(self, query: str, params: Optional[list] = None):
        ...
        if self._native_conn:
            ...
            # No DB available; provide helpful error and raise
            msg = (
                "No database connection available (native and API client missing). "
                "Attempting local CSV fallback if configured."
            )
            try:
                import streamlit as _st
                _st.warning(msg)
                ...
                pass
            # Caller may expect a cursor-like object; raise to make failure explicit
    ...
    def fetch_dataframe(self, query: str, params: Optional[list] = None) -> 'pd.DataFrame':
        ...
        if self._native_conn:
            ...
            # If no API client is configured, attempt local CSV fallback (optional)
            try:
                import pandas as pd
                ...
                if not fallback_df.empty:
                    try:
                        import streamlit as _st
                        _st.info(f"Using local CSV fallback from {self.fallback_dir} (first CSV)")
                    except Exception:
                        pass
                    return fallback_df
            except Exception:
...
def test_connection(server: str, database: str, auth: str = "windows", uid: Optional[str] = None, pwd: Optional[str] = None) -> Union[DatabaseConnection, pyodbc.Connection]:
    ...
    if is_streamlit_cloud() and not CloudConfig.USE_DIRECT_CONNECTION:
        ...
        if auth.lower() == "windows":
            conn_str = f"DRIVER={{SQL Server}};SERVER={server};DATABASE={database};Trusted_Connection=yes;MARS_Connection=Yes;"
        else:
            conn_str = f"DRIVER={{SQL Server}};SERVER={server};DATABASE={database};UID={uid};PWD={pwd};MARS_Connection=Yes;"
