...
class ChefTargetsTab:
    ...
    def render(self) -> None:
        ...
        df_join["Current"] = df_join.apply(
            lambda r: float(r.get("total_line_value_incl_tax", 0.0)) if r.get("target_type") == "Sale" else float(r.get("total_qty", 0.0)),
            axis=1,
        )
        ...
        df_join["Bonus"] = df_join.apply(
            lambda r: (0.1 * r["Target"]) if (r.get("target_type") == "Sale" and r["Achievement %"] >= 100.0) else 0.0,
            axis=1,
        )
        ...
        display = display.rename(columns={"target_type": "Type", "Current": "Current Sale"})

        st.dataframe(display.drop(columns=["Type"]), width="stretch", hide_index=True, height=420)

        # Chart
        st.markdown("---")
        st.subheader("Target vs Achievement Comparison")
        chart_df = df_join[["Product", "Target", "Current"]].copy()
        chart_df = chart_df.rename(columns={"Current": "Current Sale"})
        melt = chart_df.melt(id_vars="Product", value_vars=["Target", "Current Sale"], var_name="Type", value_name="Value")
        fig = px.bar(
            melt,
            x="Value",
            y="Product",
            ...
            barmode="group",
            height=600,
            title=f"Targets vs Sales - {month_label}",
        )
